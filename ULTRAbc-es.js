...
(async function() {
    ...
    async function runUBC() {
		...
		addMenuCheckbox(64, 64, "Activar ubicación completa en mapas: ", "mapfull3",
                    "Cuando está activado, los comandos /maproom y /mapz siempre mostrarán las coordenadas de otros jugadores en las salas con mapa, incluso si la función de compartir ubicación está bloqueada. Ten en cuenta que los administradores serán informados de que utilizas este truco si han bloqueado esta función y utilizas alguno de estos comandos. Por lo tanto, usar este truco en dichas salas puede acarrear consecuencias negativas. ¡Ten cuidado!", false, 230
                );
            ...
            PreferenceSubscreenUBCTalkingLoad = function() {
                ...
                addMenuCheckbox(64, 64, "Activar mensaje automático de AFK: ", "afkinfo",
                    "Cuando esté activada, se mostrará un mensaje automático en el chat para informar a los demás jugadores sobre tu estado AFK cuando te encuentres en dicho estado desde el tiempo que hayas configurado para esta función. También activará el icono AFK y se repetirá cada 30 minutos después del primer mensaje. Nota: Cuando ya no estés AFK, la configuración 'forzada' con el icono AFK no se modificará; deberás cambiarla manualmente si deseas que se muestre.", false, 200
                );
		...
		addMenuCheckbox(64, 64, "Habilitar el UBC guardarropa: ", "altwrobe",
                    "Si desactiva esta opción, UBC utilizará el guardarropa estándar de BC.", false, 240
                ); 
            }
    ...
    modApi.patchFunction(
        "ChatRoomSafewordRelease", {
            'ServerSend("ChatRoomChat", { Content: "ActionActivateSafewordRelease", Type: "Action", Dictionary });': 'if (Player.UBC.ubcSettings.silsafe == false) ServerSend("ChatRoomChat", { Content: "ActionActivateSafewordRelease", Type: "Action", Dictionary });',
        }
    );

    modApi.patchFunction(
        "ChatRoomSafewordRevert", {
            'ChatRoomCharacterUpdate(Player);': 'ChatRoomCharacterUpdate(Player); if (Player.UBC.ubcSettings.silsafe == false) {',
            'ServerSend("ChatRoomChat", { Content: "ActionActivateSafewordRevert", Type: "Action", Dictionary });': 'ServerSend("ChatRoomChat", { Content: "ActionActivateSafewordRevert", Type: "Action", Dictionary }); }',
            'if (Player.AllowedInteractions < AllowedInteractions.OwnerLoversWhitelistOnly) {': 'if ((Player.AllowedInteractions < AllowedInteractions.OwnerLoversWhitelistOnly) && (Player.UBC.ubcSettings.fixperm == false)) {',
        }
    );
    ...
    modApi.hookFunction('ClubCardBuilderRun', 4, (args, next) => {
        TintsEffect();
        if (ClubCardBuilderDeckIndex == -1) {
            DrawButton(1425, 25, 90, 90, "", "White", "Icons/Reset.png", "Fondo predeterminado");
            DrawButton(1540, 25, 90, 90, "", "White", "Icons/Random.png", "Fondo aleatorio");
        }
        return next(args);
    });
        ...
        if (ret) {
            ...
            if (CCPlayer.Fame >= ClubCardFameGoal) {
                ...
                if (MiniGameVictory && (ClubCardReward != null)) {
                    ...
                    ClubCardCreatePopup("TEXT", {
                        Text: Msg,
                        Button1: TextGet("Return"),
                        Function1: "ClubCardEndGame()"
                    });
                    ClubCardGameEnded = true;
                    GameClubCardReset();
                }
    ...
    modApi.patchFunction(
        "FriendListLoadFriendList", {
            "const isOnline = !!dataMap.get(memberNumber);": "if (Player.UBC.ubcSettings.nopending == true && memberName === TextGet('Unknown')) continue; const isOnline = !!dataMap.get(memberNumber);",
        }
    );
    ...
    modApi.patchFunction(
        "AsylumGGTSLoad", {
            'let Level = AsylumGGTSGetLevel(Player);': 'let Level = ""; if (Player.UBC.ubcSettings.minigame == "") { Level = AsylumGGTSGetLevel(Player); } else { Level = minigame; }',
        }
    );
    ...
    modApi.patchFunction(
        "PreferenceSubscreenChatLoad", {
            "const dropdownElements = Object.entries(PreferenceSubscreenChatDropdowns).map(([key, dropdown]) => {": "let List2 = PreferenceSubscreenChatDropdowns; if (Player.UBC.ubcSettings.alfaprf == true) List2 = Player.UBC.ubcSettings.ListChat2; const dropdownElements = Object.entries(List2).map(([key, dropdown]) => {",
            "const checkboxElements = PreferenceSubscreenChatCheckboxes.map((checkbox) => {": "let List1 = PreferenceSubscreenChatCheckboxes; if (Player.UBC.ubcSettings.alfaprf == true) List1 = Player.UBC.ubcSettings.ListChat1; const checkboxElements = List1.map((checkbox) => {",
        }
    );
    ...
    modApi.patchFunction(
        "PreferenceSubscreenExtensionsLoad", {
            'label: item.Button,': 'label: (typeof item.Button === "string" ? item.Button.replace(/Settings/gi, "").trim() : item.Button),'
        }
    );
    ...
    modApi.patchFunction(
        "PreferenceSubscreenGeneralLoad", {
            "const checkboxes = PreferenceSubscreenGeneralCheckboxes.map((checkbox) => {": "let List = PreferenceSubscreenGeneralCheckboxes; if (Player.UBC.ubcSettings.alfaprf == true) List = Player.UBC.ubcSettings.ListGeneral; const checkboxes = List.map((checkbox) => {",
        }
    );
    ...
    modApi.patchFunction(
        "PreferenceSubscreenImmersionLoad", {
            "const otherCheckboxes = PreferenceSubscreenImmersionCheckboxes.map((checkbox) => {": "let List = PreferenceSubscreenImmersionCheckboxes; if (Player.UBC.ubcSettings.alfaprf == true) List = Player.UBC.ubcSettings.ListImmersion; const otherCheckboxes = List.map((checkbox) => {",
        }
    );
    ...
    modApi.patchFunction(
        "PreferenceSubscreenMainLoad", {
            "const subscreenButtons = PreferenceSubscreens.filter(s => !s.hidden);": "let List = PreferenceSubscreens; if (Player.UBC.ubcSettings.alfmenu == true) List = Player.UBC.ubcSettings.ListAlpha; const subscreenButtons = List.filter(s => !s.hidden); ",
        }
    );
    ...
    modApi.patchFunction(
        "PreferenceSubscreenOnlineLoad", {
            "const checkboxElements = PreferenceSubscreenOnlineCheckboxes.map((checkbox) => {": "let List = PreferenceSubscreenOnlineCheckboxes; if (Player.UBC.ubcSettings.alfaprf == true) List = Player.UBC.ubcSettings.ListOnline; const checkboxElements = List.map((checkbox) => {",
        }
    );
    ...
    modApi.patchFunction(
        "PrivateClick", {
            'if (MouseIn(1885, PrivateButtonTop(7), 90, 90) && LogQuery("RentRoom", "PrivateRoom")) {': 'let backgrounds = BackgroundsPrivateRoomTagList; if (Player.UBC.ubcSettings.bgall) backgrounds = BackgroundsTagList; if (MouseIn(1885, PrivateButtonTop(7), 90, 90) && LogQuery("RentRoom", "PrivateRoom")) {',
            'BackgroundSelectionMake(BackgroundsPrivateRoomTagList, MainHallBackground, (Name, setBackground) => {': 'BackgroundSelectionMake(backgrounds, MainHallBackground, (Name, setBackground) => {',
            'BackgroundSelectionMake(BackgroundsPrivateRoomTagList, PrivateBackground, (Name, setBackground) => {': 'BackgroundSelectionMake(backgrounds, PrivateBackground, (Name, setBackground) => {',
        }
    );
        ...
        ChatSearchSearchMenu = ElementCreate({
            ...
            children: [
                ...
                (noubcbar == true) && {
                    ...
                    children: [
                        ElementCreateSettingsLabel("Tipo de sala", "chat-search-search-menu-room-type-radio-group"),
                ...
                (noubcbar == true) && {
                    ...
                    classList: ["chat-search-search-menu-grid-item"],
                    children: [
                        ElementCreateSettingsLabel("Club", "chat-search-search-menu-room-lobby-radio-group"),
                        ElementCreateRadioButtonGroup(
                ...
                // Full rooms
                {
                    ...
                    classList: ["chat-search-search-menu-grid-item"],
                    children: [
                        ElementCreateSettingsLabel("Mostrar salas completas:", "chat-search-search-menu-full-rooms-input", {
                            position: "left"
                        }),
                ...
                // Locked rooms
                {
                    ...
                    classList: ["chat-search-search-menu-grid-item"],
                    children: [
                        ElementCreateSettingsLabel("Mostrar salas bloqueadas:", "chat-search-search-menu-locked-rooms-input", {
                            position: "left"
                        }),
                ...
                // Search description
                {
                    ...
                    classList: ["chat-search-search-menu-grid-item"],
                    children: [
                        ElementCreateSettingsLabel("Descripción de la búsqueda:", "chat-search-search-menu-search-description-input", {
                            position: "left"
                        }),
                ...
                // Friends
                {
                    ...
                    classList: ["chat-search-search-menu-grid-item"],
                    children: [
                        ElementCreateSettingsLabel("Primero salas con amigos:", "chat-search-search-menu-search-friends-input", {
                            position: "left"
                        }),
                ...
                // AutoJoin feature
                {
                    ...
                    classList: ["chat-search-search-menu-grid-item"],
                    children: [
                        ElementCreateSettingsLabel("Función de unión automática:", "chat-search-search-menu-search-autojoin-input", {
                            position: "left"
                        }),
                ...
                (noubcbar == true) && {
                    ...
                    classList: ["chat-search-search-menu-grid-item"],
                    children: [
                        ElementCreateSettingsLabel("Juego", "chat-search-search-menu-room-game-dropdown"),
                ...
                // Room size
                {
                    ...
                    classList: ["chat-search-search-menu-grid-item"],
                    children: [
                        ElementCreateSettingsLabel("Tamaño de la sala", "chat-search-search-menu-room-size-grid"),
                        {
                            tag: "div",
                            attributes: {
        ...
        targetCard.OnPlay = function(CCPlayer) {
            if (CCPlayer.Control == 'AI') {
                ...
                if (ClubCardSelection == null && CCPlayer.DiscardPile.filter(card => card.Type !== "Event").length > 0) {
                    ClubCardCreatePopup("SEARCH", {
                        CardsPool: CCPlayer.DiscardPile.filter(card => card.Type !== "Event")
                    });
                    return;
                }
    ...
    function IsBcxWhisperAllowed(target) {
        ...
        let rg3 = 0;
        let rg4 = 0;
        const ownershipMember = (Player.Ownership && !Player.Ownership.Name.startsWith("NPC")) ? [Player.Ownership.MemberNumber] : [];
    ...
    CommandCombine([{
        ...
        Action: (args) => {
            if (args === "") {
                ...
                if (heart != 0) {
                    infomsg("Candados de corazón aplicados y configurados.");
                    ChatRoomCharacterUpdate(Player);
    ...
    CommandCombine([{
        ...
        Action: (args) => {
            ...
            if (args === "") {
                let msg = "El comando bg1 debe ir seguido de un número. Lista de fondos disponibles:\n" +
                    "BONDAGE COLLEGE\n" +
    ...
    CommandCombine([{
        Tag: 'cname',
        Description: "(nombre personalizado): crea un nombre personalizado que no requiere cumplir condiciones. .",
        Action: (args, originalInput) => {
            if (args === "") {
                let msg = "El comando cname debe ir seguido del nombre personalizado que quieras mostrar en tu perfil.\n" +
                    "Se aceptan más letras que para el nombre oficial; también puede incluir espacios y números.\n" +
                    "¡Máximo 20 caracteres (espacios incluidos)!\n" +
                    "Solo funcionará entre usuarios de UBC y reemplazará el nombre oficial en el perfil.\n" +
                    "Usa 'None' como nombre para volver a un perfil sin nombre personalizado.";
                infomsg(msg);
                return;
            }
            ...
            if ((length > 0) && (length < 21) && (custom.match(LS))) {
                ...
                infomsg("¡Máximo 20 caracteres (espacios incluidos)!");
            }
        }
    }])
        ...
        Action: (args, originalInput) => {
            if (args === "") {
                let msg = "El comando ctitle debe ir seguido del título personalizado que quieras mostrar en tu perfil.\n" +
                    ...
                    "Solo funcionará entre usuarios de UBC.\n" +
                    ...
                    if (ChatRoomData.MapData.Type === "Always") {
                        ...
                        if (character.OnlineSharedSettings?.UBCShared?.Inmap != undefined) {
                           ...
                           if (typeof ubcshared.Inmap === "boolean") {
							   exinfo = `Presencia real en el mapa: ${ubcshared.Inmap ? "SÍ" : "NO"}`;
                            } else {
        ...
        Action: (args) => {
            ...
            if (args === "admin") {
                let msg = "Comandos de Admin\\n" +
                    ...
                    "<b>/bg1</b> (número) = utiliza un fondo personalizado. /bg1 para ver la lista.\n" +
            ...
            if (args === "clothing") {
                let msg = "Comandos de ropa - * = más información al usar\n" +
                    ...
                    "<b>/wexport</b> (objetivo) = exporta completamente vestimenta en formato UBC/BCG.\n" +
                    "<b>/wimport1</b> (objetivo) = importa ropa + ataduras en formato UBC/BCG.\n" +
                    "<b>/wimport2</b> (objetivo) = importa ropa + ataduras + cosplay en formato UBC/BCG.\n" +
                    "<b>/wimport3</b> (objetivo) = importa completamente vestimenta en formato UBC/BCG.\n" +
    ...
    CommandCombine([{
        Tag: 'wimport1',
        Description: "(objetivo): importa ropa + ataduras en formato UBC/BCG..",
        Action: (args) => {
            let target = Player;
            if (args != "") target = TargetSearch(args);
            if (!target) return;
    ...
    CommandCombine([{
        Tag: 'wimport2',
        Description: "(objetivo): importa ropa + ataduras + cosplay en formato UBC/BCG. .",
        Action: (args) => {
            let target = Player;
            if (args != "") target = TargetSearch(args);
            if (!target) return;
    ...
    CommandCombine([{
        Tag: 'wimport3',
        Description: "(objetivo): importa completamente vestimenta en formato UBC/BCG.",
        Action: (args) => {
            let target = Player;
            if (args != "") target = TargetSearch(args);
            if (!target) return;
