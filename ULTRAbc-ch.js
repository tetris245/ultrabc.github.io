...
(async function() {
		...
		PreferenceSubscreenResetExit = function () {
            ...
            PreferenceSubscreenUBCCheatsLoad = function() {
                ...
                addMenuButton(150, 64, "添加/移除卡牌游戏的额外卡牌：", "Toggle", function() {
                    ...
                    "此设置为切换开关。您可以添加或移除所有奖励额外卡牌。请注意，通过正常方式获得的额外卡牌也会在此设置切换到移除操作时被移除。", 160
                );
    ...
    CommandCombine([{
        Tag: 'bg4',
        Description: "（屏幕）（背景）：为俱乐部卡牌游戏、好友列表、主大厅、私人房间（单人模式）或计时牢房选择标准背景",
        Action: (args) => {
            if (args === "") {
                let msg = "bg4 命令后必须跟两个数字：\n" +
                    "- 第一个数字表示相关屏幕：\n" +
                    "0 = 角色信息 - 1 = 聊天搜索\n" +
                    "2 = 俱乐部纸牌游戏 - 3 = 好友列表\n" +
                    "4 = 主大厅 - 5 = 私人房间（SP）\n" +
                    ...
                    " \n" +
                    "- 第二个数字范围是 -1 到某个最大值：\n" +
                    "不使用 BCX：0 到 235 为官方 BC 背景，如果你使用 /bg1 命令则添加 236 到 318。\n" +
                    "使用 BCX：0 到 235 为官方 BC 背景，236 到 325 由 BCX 添加，如果你使用 /bg1 命令则添加 326 到 399（某些背景有两个数字）。\n" +
                    " \n" +
                    "使用 -1 可返回默认背景。提示：使用 </b>/bglist</b> 可查看哪个数字对应哪个背景。";
                infomsg(msg);
            } else {
                let stringBg1 = args;
                ...
                if ((screen > -1) && (screen < 9)) {
                    ...
                    if (screen == 1) {
                        ...
                        if ((csbg > -2) && (csbg < (BackgroundsList.length - 1))) {
                            ...
                            csname = csback;
                            M_MOANER_saveControls();
                            let msg = "聊天搜索的背景现在是：" + BackgroundsTextGet(csname) + "。";
                            infomsg(msg);
                        }
                    }
                    if (screen == 2) {
                        ...
                        if ((ccbg > -2) && (ccbg < (BackgroundsList.length - 1))) {
                            ...
                            let msg = "俱乐部纸牌游戏的背景现在是：" + BackgroundsTextGet(ccback) + "。";
                            infomsg(msg);
                        }
                    }
                    if (screen == 3) {
                        ...
                        if ((frbg > -2) && (frbg < (BackgroundsList.length - 1))) {
                            ...
                            frname = frback;
                            M_MOANER_saveControls();
                            let msg = "好友列表的背景现在是：" + BackgroundsTextGet(frname) + "。";
                            infomsg(msg);
                        }
                    }
                    if (screen == 4) {
                        ...
                        if ((mhbg > -2) && (mhbg < (BackgroundsList.length - 1))) {
                            ...
                            let msg = "主大厅的背景现在是：" + BackgroundsTextGet(mhback) + "。";
                            infomsg(msg);
                        }
                    }
                    if (screen == 5) {
                        ...
                        if ((prbg > -2) && (prbg < (BackgroundsList.length - 1))) {
                            ...
                            let msg = "你的私人房间（SP）背景现在是：" + BackgroundsTextGet(prback) + "。";
                            infomsg(msg);
                        }
                    }
                    if (screen == 6) {
                    ...
                    if (screen == 8) {
                        ...
                        if ((drbg > -2) && (drbg < (BackgroundsList.length - 1))) {
                            ...
                            let msg = "创建新房间时的默认背景现在是：" + BackgroundsTextGet(drback) + "。";
                            infomsg(msg);
                        }
                    }
                }
        ...
        Action: (args) => {
            ...
            if (args === "settings") {
                let msg = "设置命令 - * = 使用时获取更多信息\n" +
                    "<b>/bg4</b> （屏幕）（背景）= 为某些特定的 BC 房间选择标准背景。*\n" +
